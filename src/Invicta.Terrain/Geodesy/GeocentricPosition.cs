...
internal readonly record struct GeocentricPosition(double X, double Y, double Z)
{
    ...
    public double Length => double.Sqrt(Dot(this));
    ...
    {
        double latitude = coordinate.Latitude * double.Pi / 180;
        double longitude = coordinate.Longitude * double.Pi / 180;
        double sinLatitude = double.Sin(latitude);
        double cosLatitude = double.Cos(latitude);

        // The prime vertical radius of curvature.
        double n = Wgs84.EquatorialRadius / double.Sqrt(1 - (EccentricitySquared * sinLatitude * sinLatitude));

        return new GeocentricPosition(
            (n + height) * cosLatitude * double.Cos(longitude),
            (n + height) * cosLatitude * double.Sin(longitude),
    ...
    public static GeocentricPosition Up(GeoCoordinate coordinate)
    {
        ...
        return new GeocentricPosition(
            double.Cos(latitude) * double.Cos(longitude),
            double.Cos(latitude) * double.Sin(longitude),
            double.Sin(latitude));
    }
